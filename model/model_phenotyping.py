...
def get_loss(model, pred, true, mask, gamma=0.9, entropy_term=0.01, use_baseline=True, device=None):
    task_pred = pred * mask
    task_true = true.unsqueeze(1)
    task_true = task_true.repeat(1, pred.size(1), 1)
    indicator = mask.squeeze(-1).sum(dim=-1).long() - 1
    batch_idx = torch.arange(pred.size(0), dtype=torch.long).to(device)
    pred = pred[batch_idx, indicator, :]
    bce_loss = nn.BCELoss(reduction='none')
    loss_task = bce_loss(task_pred, task_true).mean(dim=-1)
    loss_task = (loss_task * mask.squeeze(-1)).sum(dim=-1) / mask.sum(dim=1)
