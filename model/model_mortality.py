...
def get_loss(model, pred, true, gamma=0.9, use_baseline=True, entropy_term=0.01, device=None):
    loss_task = torch.nn.BCELoss()(pred, true)
    
    act_prob1 = model.agent1_prob
    act_prob1 = torch.stack(act_prob1).permute(1, 0).to(device) #B*T
    act_entropy1 = model.agent1_entropy
    act_entropy1 = torch.stack(act_entropy1).permute(1, 0).to(device) #B * T
    if use_baseline == True:
        act_baseline1 = model.agent1_baseline
        act_baseline1 = torch.stack(act_baseline1).squeeze(-1).permute(1, 0).to(device) #B*T

    act_prob2 = model.agent2_prob
    act_prob2 = torch.stack(act_prob2).permute(1, 0).to(device) #B*T
    act_entropy2 = model.agent2_entropy
    act_entropy2 = torch.stack(act_entropy2).permute(1, 0).to(device) #B * T
    if use_baseline == True:
        act_baseline2 = model.agent2_baseline
        act_baseline2 = torch.stack(act_baseline2).squeeze(-1).permute(1, 0).to(device) #B*T
    ...
    rewards = torch.stack(running_rewards).permute(1, 0) # B * T
    rewards = (rewards - rewards.mean(dim=1).unsqueeze(-1)) / (rewards.std(dim=1) + 1e-7).unsqueeze(-1)
    rewards = rewards.detach()
    
    time_step = act_prob1.size(1)
    if use_baseline == True:
        loss_value1 = torch.sum((rewards - act_baseline1) ** 2, dim=1) / time_step
        loss_value1 = torch.mean(loss_value1)
        loss_value2 = torch.sum((rewards - act_baseline2) ** 2, dim=1) / time_step
        loss_value2 = torch.mean(loss_value2)
        loss_value = loss_value1 + loss_value2
        loss_RL1 = -torch.sum(act_prob1 * (rewards - act_baseline1) + entropy_term * act_entropy1, dim=1) / time_step
        loss_RL1 = torch.mean(loss_RL1)
        loss_RL2 = -torch.sum(act_prob2 * (rewards - act_baseline2) + entropy_term * act_entropy2, dim=1) / time_step
        loss_RL2 = torch.mean(loss_RL2)
        loss_RL = loss_RL1 + loss_RL2
        loss = loss_RL + loss_task + loss_value
    else:
        loss_RL1 = -torch.sum(act_prob1 * rewards + entropy_term * act_entropy1, dim=1) / time_step
        loss_RL1 = torch.mean(loss_RL1)
        loss_RL2 = -torch.sum(act_prob2 * rewards + entropy_term * act_entropy2, dim=1) / time_step
        loss_RL2 = torch.mean(loss_RL2)
        loss_RL = loss_RL1 + loss_RL2
